...
internal class BrowserPlatform(private val activity: ComponentActivity) {
    ...
    private val displayOverrides = java.util.WeakHashMap<WebView, Boolean>()
    ...
    fun handleNavigation(view: WebView, request: WebResourceRequest): Boolean {
        ...
        if (uri.scheme.equals("https", true) || uri.scheme.equals("http", true)) {
            if (request.isForMainFrame && applyDisplayPolicy(view, uri.toString())) {
    ...
    /** Returns true when UA changed and the caller must restart the main-frame navigation. */
    fun applyDisplayPolicy(view: WebView, url: String): Boolean {
        val wantsDesktop = displayOverrides[view] ?: SiteDisplayPolicy.prefersDesktop(url)
        val original = desktopViews[view]
        if (wantsDesktop && original == null) {
    ...
    fun isDesktop(view: WebView): Boolean = desktopViews.containsKey(view)

    fun toggleDesktop(view: WebView): Boolean {
        hideFullscreen()
        imageSearchViews.remove(view)?.let { view.settings.userAgentString = it }
        imageOverview.remove(view)?.let { view.settings.loadWithOverviewMode = it }
        val original = desktopViews.remove(view)
        if (original != null) {
            displayOverrides[view] = false
            view.settings.userAgentString = original
            view.settings.loadWithOverviewMode = false
            message("モバイル表示")
        } else {
            displayOverrides[view] = true
            desktopViews[view] = view.settings.userAgentString
            view.settings.userAgentString = desktopAgent(view.settings.userAgentString)
            view.settings.loadWithOverviewMode = true
        ...
        view.setOnTouchListener { _, event ->
            when (event.actionMasked) {
                android.view.MotionEvent.ACTION_DOWN -> {
                    ctrlGesture = ctrlDown || event.metaState and android.view.KeyEvent.META_CTRL_ON != 0
                ...
                android.view.MotionEvent.ACTION_UP -> {
                    ...
                    val intercept = ctrlGesture && !moved && isLink
                    ctrlGesture = false
                    if (intercept) {
                ...
                android.view.MotionEvent.ACTION_CANCEL -> { ctrlGesture = false; false }
                else -> false
            }
        }
