...
- URL／検索語を入力できるアドレスバー
- 戻る、進む、再読み込み／停止、ホーム
- Sleipnirの一般的な操作パターンを参考にした画面下部のカラータブ棚（作成、切り替え、個別終了、選択中タブの明示）
...
タブレットでWebページの縦方向を広く使えるよう、アドレス入力、再読み込み、サイト設定、メニューは上部の1行にまとめています。タブはSleipnirの画面構成を参考にしつつ独自に実装した画面下部の横スクロール棚へ表示します。選択中のタブは幅、影、色、マーカーで区別でき、各タブの`×`で終了、右端の`＋`で作成できます。戻る、ホーム、進むも親指で操作しやすいようタブ棚の下に配置しています。製品固有のロゴ、画像、コードは使用していません。
