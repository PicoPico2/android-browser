...
- サイトごと・プロフィールごとの3段階通信ブロック（オフ／標準／強力）
- Web動画向けの自動再生、互換混在コンテンツ、JavaScriptウィンドウ設定
...
通信ブロックはアプリ同梱の小さなドメイン一覧だけを使い、外部サーバーへ閲覧情報を送りません。「標準」は既知の広告・追跡ドメインを保守的に遮断し、「強力」は対象を広げます。Google、reCAPTCHA、hCaptcha、主要な決済ドメインと、URLパスにログイン・認証・決済を示す語がある通信は遮断対象から除外しています。それでもサイトが壊れる場合は、アドレスバー横の「盾」からそのサイトだけ「オフ」にして再読み込みできます。設定はプロフィールとサイトの組み合わせごとに保存されます。

### Web動画について

ニコニコ動画などのHTML5動画との互換性のため、JavaScript、DOM Storage、Cookie、メディアの自動再生、互換モードの混在コンテンツを許可しています。固定User-AgentやページへのJavaScript注入は行いません。動画が黒画面になる場合は、まずサイト別通信ブロックを「オフ」にして再読み込みし、Android System WebViewを最新版へ更新してください。DRM、端末の動画デコーダー、サイト側仕様に起因する再生不能はアプリから回避しません。

### パスワードについて

パスワード保存は引き続き**未実装**です。平文保存やWebView DOMへの安易な注入は追加していません。Android Keystore、AES-GCM、端末認証、画面・ログ・バックアップ対策を含む設計とテストが完成するまで有効化しません。メニューにも未実装であることを表示します。
